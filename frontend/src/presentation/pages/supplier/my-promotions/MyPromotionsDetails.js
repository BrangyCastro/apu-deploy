...
const MyPromotionsDetails = ({ match }) => {
  ...
  const getPromotionsId = async (id) => {
    ...
    try {
      const response = await getPromotionsByUserApi(id);
      setState((old) => ({
        ...old,
        promotions: response,
        loading: false,
      ...
      {state.error ? (
        ...
        <>
          {state.loading || state.loadingVendors ? (
            ...
            <CCard>
              <CCardBody>
                <EditPromotionsForm data={state} reload={reload} />
              </CCardBody>
            </CCard>
          )}
        </>
...
export default withRouter(MyPromotionsDetails);
